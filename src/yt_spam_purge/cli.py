...
def main():
    ...
    if not os.path.exists(spamListDict['Meta']['VersionInfo']['Path']):
        files.copy_asset_file(spamListDict['Meta']['VersionInfo']['FileName'],
    ...
    def primaryInstance(miscData):
        ...
        while validMode == False:
            ...
            validModeValues = ['1', '2', '3', '4', '5', '6', '7', '8', '9', 'chosenvideos', 'recentvideos',
        ...
        if userNotChannelOwner == True and moderator_mode == False:
            print(
                f" {F.LIGHTRED_EX}Note: With 'Not Your Channel Mode' enabled, you can only report matched comments while using 'Auto-Smart Mode'.{S.R}")  # Based on filterModesAllowedforNonOwners
        elif userNotChannelOwner == True and moderator_mode == True:
            print(
        ...
        if scanMode == "communityPost" or scanMode == "recentCommunityPosts":
            def scan_community_post(config, communityPostID, limit, postScanProgressDict=None, postText=None):
                ...
                for key, value in allCommunityCommentsDict.items():
                    ...
                    progressStats = f"[ {str(scannedCount)} / {str(retrievedCount)} ]".ljust(15,
                                                                                             " ") + f" ({percent:.2f}%)"
            ...
            if scanMode == "communityPost":
                ...
                for post in recentPostsListofDicts:
                    ...
                    scan_community_post(config, id, maxScanNumber, postScanProgressDict=postScanProgressDict,
                                        postText=postText)
            ...
            def scan_video(miscData, config, filtersDict, scanVideoID, videosToScan=None, currentVideoDict=None,
                ...
                while nextPageToken != "End" and current.scannedCommentsCount < maxScanNumber:
                    nextPageToken, currentVideoDict = operations.get_comments(current, filtersDict, miscData, config,
                                                                              currentVideoDict, scanVideoID,
                                                                              nextPageToken, videosToScan=videosToScan)
                    if nextPageToken == "Error":
                        return "Error"
                return "OK"
        ...
        logFileContents, logMode = logging.print_comments(current, config, scanVideoID,
                                                          list(current.matchedCommentsDict.keys()), loggingEnabled,
                                                          scanMode, logMode)
        ...
        if confirmDelete == None and returnToMenu != True:
            ...
            while confirmDelete != "DELETE" and confirmDelete != "REPORT" and confirmDelete != "HOLD":
                ...
                if confirmDelete == "DELETE" and userNotChannelOwner == False:
                    ...
                    current, excludedCommentsDict, authorsToExcludeSet, commentIDExcludeSet, = operations.exclude_authors(
        ...
        if loggingEnabled:
            ...
            logging.write_log_completion_summary(current, logMode, banChoice=False,
        ...
        if (
                confirmDelete == "DELETE" or confirmDelete == "REPORT" or confirmDelete == "HOLD") and deletionEnabled == True and current.errorOccurred == False:
            banChoice = False
