  ...
  try:
    ...
    # Get all comment threads across the whole channel
      ...
      results = (
        ...
        .list(part="snippet, replies", allThreadsRelatedToChannelId=auth.CURRENTUSER.id, maxResults=100, pageToken=nextPageToken, fields=fieldsToFetch, textFormat="plainText")
        .execute()
      )
...
def check_duplicates(current, config, miscData, allVideoCommentsDict, videoID):
  ...
  for authorID, authorCommentsList in allVideoCommentsDict.items():
    ...
    if (
      ...
      or any(authorID == value['authorID'] for key, value in current.matchedCommentsDict.items())
    ):
      scannedCount += 1
