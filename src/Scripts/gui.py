...
from tkinter import END, Button, Canvas, Entry, Event, Menu, PhotoImage, TclError, Text, Tk, Widget
...
class CanvasObject:
    ...
    def __init__(self, canvas: Canvas, canvas_id: int):
    ...
    def itemconfig(self, **kwargs: Any):
...
def rClicker(e: Event):
    #''' right click context menu for all Tk Entry and Text widgets'''
    try:

        def rClick_Copy(e: Event, _apnd=0):  # type: ignore
            e.widget.event_generate('<Control-c>')

        def rClick_Cut(e: Event):
            e.widget.event_generate('<Control-x>')

        def rClick_Paste(e: Event):
...
returnText: set[str] = set()


# Function that is called by main program that brings up window to take in user input
def take_input_gui(mode: str, stripLettersNumbers: bool = False, stripKeyboardSpecialChars: bool = False, stripPunctuation: bool = False):
    ...
    window.iconphoto(False, PhotoImage(file=resource_path(Path("icon.png"))))
    ...
    canvas.create_text(19.0, 48.0, anchor="nw", text="Enter Search Terms:", fill="#000000", font=("Roboto", 12 * -1))

    canvas.create_text(19.0, 148.0, anchor="nw", text="Terms will be displayed back here:", fill="#000000", font=("Roboto", 12 * -1))

    # Bottom text box
    entry_image_1 = PhotoImage(file=resource_path(Path("outputTextBox.png")))
    _entry_bg_1 = canvas.create_image(138.5, 186.0, image=entry_image_1)  # type: ignore
    outputTextBox = Text(
        bd=0,
        bg="#F8F8F8",
        state="disabled",
    ...
    entry_image_2 = PhotoImage(file=resource_path(Path("inputTextBox.png")))
    _entry_bg_2 = canvas.create_image(138.5, 85.0, image=entry_image_2)  # type: ignore
    inputTextBox = Entry(bd=0, bg="#FCFCFC", highlightthickness=0)
    inputTextBox.place(x=27.0, y=67.0, width=223.0, height=30.0)

    button_image_1 = PhotoImage(file=resource_path(Path("cancelButton.png")))
    cancelButton = Button(image=button_image_1, borderwidth=0, highlightthickness=0, command=quit, relief="flat")
    cancelButton.place(
        x=152.0,
        y=225.0,
        # width=78.0,
    ...
    button_image_2 = PhotoImage(file=resource_path(Path("executeButton.png")))
    executeButton = Button(image=button_image_2, borderwidth=0, highlightthickness=0, command=execute, relief="flat")
    executeButton.place(
        x=42.0,
        y=225.0,
        # width=78.0,
    ...
    canvas.create_text(32.0, 8.0, anchor="nw", text="Filter Terms Input", fill="#000000", font=("Roboto", 24 * -1))

    # Submit Button
    button_image_3 = PhotoImage(file=resource_path(Path("inputSubmitButton.png")))
    inputSubmitButton = Button(image=button_image_3, borderwidth=0, highlightthickness=0, command=partial(submit, inputTextBox), relief="flat")
    ...
    warningMessage = CanvasObject(canvas, canvas.create_text(19.0, 105.0, anchor="nw", text="No valid terms entered!", fill="red", font=("Roboto", 12 * -1), state="hidden"))
