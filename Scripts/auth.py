...
def get_authenticated_service():
    ...
    if not os.path.exists(CLIENT_SECRETS_FILE):
        ...
        print(
            f"  > For instructions on how to get an API key, visit: {F.YELLOW}www.TJoe.io/api-setup{S.R}"
        )
        print(
...
def get_current_user(config):
    ...
    if len(results) == 0:  # Check if results are empty
        ...
        print(
            "> The client_secrets file does not match user authorized with token.pickle file."
        )
        print(
